...
export function ConfirmPage() {
  ...
  return <StepPage id="confirm" summary="Writes the chosen designs into one CDX1, re-runs them in RASAero (the native engine, or the VM) and compares its apogees with the optimizer's."
    action={<ActionPanel vm prereqs={[{ label: `${d.n || 0} designs`, ok: d.n > 0, hint: "run the optimizer first (step 5)" }, { label: "RASAero engine", ok: s.engine?.ok ? true : "warn", hint: s.engine?.ok ? "" : "engine not built: the VM worker will be used" }]}
    ...
    how={<p>One batched CDX1 with the chosen rows is flown by RASAero's engine (or, as a fallback, the VM worker: File → Open, Rerun All Simulations, Save As); RASAero's MaxAltitude comes back per row and the comparison is stored in <code>output/confirm.csv</code>. With the shortlist the exact starred designs are sent (<code>rpa confirm --designs …</code>).</p>}>
    <Card><CardTitle right={c.file.exists ? ago(c.file.mtime) : ""}>RASAero vs the optimizer</CardTitle>
      {c.rows.length ? <div><StatList className="mb-3"><Stat label="designs confirmed" value={c.rows.length} /><Stat label="max |difference|" value={fmt(maxDiff, 2)} unit="%" cls={maxDiff < 1 ? "ok" : "warn"} /><Stat label="mean difference" value={fmt(c.rows.reduce((a: number, r: any) => a + (r.diff_pct || 0), 0) / c.rows.length, 2)} unit="%" /></StatList>
        <DataTable columns={["booster", "sustainer", "profile", "sep_delay_s", "ign_delay_s", "apogee_python_ft", "apogee_rasaero_ft", "diff_ft", "diff_pct", "max_vel_rasaero_fps", "t_apogee_rasaero_s"]} rows={c.rows} labels={{ sep_delay_s: "sep [s]", ign_delay_s: "ign [s]", apogee_python_ft: "optimizer apogee", apogee_rasaero_ft: "RASAero apogee", diff_ft: "Δ ft", diff_pct: "Δ %", max_vel_rasaero_fps: "RASAero max vel", t_apogee_rasaero_s: "RASAero t apogee" }} format={{ apogee_python_ft: (v) => fmt(v, 0), apogee_rasaero_ft: (v) => fmt(v, 0), diff_ft: (v) => (isNum(v) ? (v > 0 ? "+" : "") + fmt(v, 0) : "—"), diff_pct: (v) => (isNum(v) ? (v > 0 ? "+" : "") + v.toFixed(2) : "—") }} tools={{ csv: "confirm.csv" }} /></div> : <div className="text-muted">nothing confirmed yet</div>}</Card>
  </StepPage>;
}
