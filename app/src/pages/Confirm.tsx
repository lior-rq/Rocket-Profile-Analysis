...
export function ConfirmPage() {
  ...
  return <StepPage id="confirm" summary="Writes the chosen designs into one CDX1, re-runs them in RASAero (the native engine, or the VM) and compares its apogees with the optimizer's."
    action={<ActionPanel vm prereqs={[{ label: `${d.n || 0} designs`, ok: d.n > 0, hint: "run the optimizer first (step 4)" }, { label: "RASAero engine", ok: s.engine?.ok ? true : "warn", hint: s.engine?.ok ? "" : "engine not built: the VM worker will be used" }]}
