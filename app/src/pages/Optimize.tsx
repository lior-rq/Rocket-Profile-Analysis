...
export function DiskCard() {
  ...
  const dk = s?.disk;
  if (!dk) return null;
  const clean = async (what: string, older?: number) => { try { const r = await api("/api/cleanup", { what, older_days: older }); toast.success(`removed ${r.removed} item(s), ${sizeFmt(r.bytes)}`); refresh(); } catch (e: any) { toast.error(e.message); } };
  ...
  return <div><h3 className="mb-1 flex justify-between text-[13px] font-semibold">Disk <span className="font-normal text-muted">{sizeFmt(dk.histories.bytes + dk.jobs.bytes + dk.search_rows.bytes)} in the bulky outputs</span></h3>
    ...
    <Row label={`${dk.jobs.folders} job folder(s) for the VM worker`} info={dk.jobs} btn={dk.jobs.folders ? <ConfirmButton label="Delete finished jobs older than 7 days" armedLabel="Click again to delete" onClick={() => clean("jobs", 7)} /> : null} />
...
export function OptimizePage() {
  ...
  if (!s || !r) return null;
  const o = s.optimize, cfg = s.config;
  const args: string[] = [];
  ...
  const backendNote = backend === "rasaero_native" ? <Callout kind="info">RASAero's own engine flies every simulation on this machine, in parallel host processes. Exact RASAero numbers, no validation needed.</Callout> : backend === "rasaero" ? <Callout kind="info">RASAero backend: every simulation batch goes through the VM worker — expect hours for 60 boosters.</Callout> : <Callout kind="info">OpenRocket backend: preview only, its aerodynamics differ from RASAero.</Callout>;
  ...
  return <StepPage id="optimize" summary={`For every booster: one long-coast flight characterizes the attached stack and decides which profiles it is eligible for; then, for each eligible (booster, profile), separation delays ${fmt(cfg.profiles.separation_delay_min_s)}–${fmt(cfg.profiles.separation_delay_max_s)} s and ignition delays ${fmt(cfg.profiles.ignition_delay_min_s)}–${fmt(cfg.profiles.ignition_delay_max_s)} s are searched until the apogee hits ${fmt(cfg.target.apogee_ft, 0)} ± ${fmt(cfg.target.tolerance_ft, 0)} ft. Solutions are re-flown with a full time history and checked against the profile rule.`}
    action={<ActionPanel prereqs={[{ label: "inputs checked", ok: s.inputs.status === "ok" ? true : s.inputs.status === "error" ? false : "warn", hint: s.inputs.status === "error" ? "fix the input problems in step 1" : "inputs not checked since they changed" }, backend === "rasaero_native" ? { label: "RASAero engine", ok: s.engine?.ok ? true : false, hint: s.engine?.detail } : { label: "VM worker", ok: "warn", hint: "the VM worker must be running" }, { label: `mass ${s.mass.hardware_mass_lb != null ? s.mass.hardware_mass_lb + " lb" : ".ork"}`, ok: true }]}
      ...
      options={<div className="flex flex-col gap-2"><div className="flex flex-wrap gap-3"><Checkbox label="Fresh run (recompute cached stages)" checked={ps.fresh} onChange={(e) => setPs({ ...ps, fresh: e.target.checked })} /><Checkbox label="Export a full time history for every design, not only the solved ones" checked={ps.unsolved} onChange={(e) => setPs({ ...ps, unsolved: e.target.checked })} /><Checkbox label='Also try "decelerate below Mach 0.9, then separate"' checked={ps.decel} onChange={(e) => setPs({ ...ps, decel: e.target.checked })} /></div>
        <div className="flex flex-wrap items-center gap-3 text-[13px]"><label className="inline-flex items-center gap-1">backend <Select className="h-7" value={ps.backend} onChange={(e) => setPs({ ...ps, backend: e.target.value })}><option value="">config ({cfg.backend})</option><option value="rasaero_native">rasaero_native</option><option value="rasaero">rasaero (VM)</option><option value="openrocket">openrocket (preview)</option></Select></label><label className="inline-flex items-center gap-1">only the first <Input type="number" min={1} className="h-7 w-16" value={ps.limit} placeholder="all" onChange={(e) => setPs({ ...ps, limit: e.target.value })} /> boosters</label><label className="inline-flex items-center gap-1">only <Input className="h-7 w-[200px]" value={ps.boosters} placeholder="e.g. 01,32,60 or labels" onChange={(e) => setPs({ ...ps, boosters: e.target.value })} /></label></div></div>}
