...
import { Callout, EngineCard, KV, RunHistory, Stat, StatList } from "@/components/common";
...
export function OverviewPage() {
  ...
  return <div className="flex flex-col gap-4">
    ...
    <Card><CardTitle>Progress</CardTitle><div className="grid grid-cols-2 gap-2 md:grid-cols-4">{STEPS.map((x) => <Link key={x.id} to={x.path} className="rounded-md border border-line bg-panel-2 p-2 hover:border-accent"><div className="text-[10px] uppercase text-muted">step {x.n}</div><div className="text-[13px] font-medium leading-4">{x.title}</div><div className="mt-1"><Badge status={st(x.id)} /></div></Link>)}</div></Card>
