...
export function ReferencePage() {
  ...
  return <StepPage id="reference" summary="Optional. Exports a few complete flights from RASAero and stores the full View Data exports; step 3 checks that the bundled engine reproduces them. Only exports made by the RASAero GUI in the VM (engine: vm) are an independent reference — the native engine reproducing its own exports proves nothing. Cases spread over the booster set and over short and long coasts."
    action={<ActionPanel vm prereqs={[{ label: "mass table", ok: !!(s.mass.table && !s.mass.table.error), hint: "computed on demand from the .ork" }, { label: "RASAero GUI (VM)", ok: native ? "warn" : true, hint: native ? "the native engine will export these; set rasaero.engine: vm for an independent reference" : "" }]}
      ...
      notes={<>{r.n && r.n < 5 ? <Callout kind="warn">{r.n} case(s) so far — at least 5 give a meaningful validation.</Callout> : null}{r.stale ? <Callout kind="warn">The CDX1 is newer than these flights — export fresh ones if the vehicle changed.</Callout> : null}</>} cmd={<CmdPreview stage="reference" args={["--cases", String(cases)]} />} />}
    how={<p>Each case is one RASAero simulation row (booster, separation delay, ignition delay) with the masses from the mass table. The engine flies it and writes the full time history at 0.01 s; the export and the row inputs are stored side by side in <code>{r.dir}</code>.</p>}>
    <Card><CardTitle>Reference cases ({r.n})</CardTitle>
      ...
      <KV pairs={[["delay convention", "separation delay from burnout; ignition delay from separation (RASAero)"]]} /></Card>
