...
function CustomizeTab(p: P) {
  ...
  return <div className="flex flex-col gap-3">
    <div className="flex flex-wrap items-center gap-2"><label className="inline-flex items-center gap-2 text-[12px]">design <Select className="h-7 max-w-[520px]" value={r.key} onChange={(e) => setSearch({ sel: e.target.value })}>{sorted.map((x: any) => <option key={x.key} value={x.key}>{x.booster} + {x.sustainer} · {x.profile} · {x.status} · Δ {signed(x.dev_ft)} ft</option>)}</Select></label><Badge status={STATUS_BADGE[r.status] || "todo"}>{r.status}</Badge><Button size="sm" variant="ghost" onClick={() => selectDesign(r.key)}>open in Designs →</Button><span className="flex-1" /><span className="text-[12px] text-muted">{fq.isFetching ? "simulating…" : flight ? "flown on the native engine · estimate, not verified" : ""}</span></div>
    <div className="grid gap-2 md:grid-cols-3">
      ...
      <ParamSlider label="hardware (dry) mass [lb]" hint={manual ? "manual mass model: set the masses in config" : !mt ? (s.mass.table ? "mass table unreadable" : "no mass table yet") : baseMass == null ? "no dry masses in the mass table: recompute it" : "both stages scale with it"} value={q.mass} min={1} max={baseMass ? Math.round(baseMass * 1.5) : 100} step={1} base={baseMass != null ? `mass table: ${fmt(baseMass, 1)}${isNum(mt?.hardware_mass_lb) ? "" : " (.ork)"}` : null} onChange={(v) => setQ({ ...q, mass: v })} disabled={manual || baseMass == null} />
    </div>
    ...
    {s.mass.table?.error ? <Callout kind="err">mass table unreadable ({s.mass.table.error}): recompute it (Inputs → Mass)</Callout> : !mt ? <Callout kind="info">no mass table yet: run the mass stage (Inputs → Mass) to fly custom delays</Callout> : null}
