...
import { HistoryChart, HistoryOverlay, LineChart, PALETTE, ScatterChart, type HistoryFrame } from "@/components/charts";
import { FlightConfig, type Player } from "@/components/player";
...
function DesignDetail({ r, nav, s, d, cfg, shortlist, setShortlist, setSearch }: P & { r: any; nav: any }) {
  ...
  const samples = useCached<any>("samples:" + key, "/api/samples?key=" + encodeURIComponent(key), d.mtime);
  ...
  const samplesChart = () => { if (!samples.data) return <div className="text-muted">Loading search samples…</div>; const pts = (samples.data[key] || []).filter((p: any) => isNum(p[1])); if (!pts.length) return <div className="text-[12px] text-muted">no search samples for this candidate</div>; const bySep = new Map<number, any[]>(); for (const p of pts) { const sep = p.length > 2 ? p[2] : r.sep_delay_s; if (!bySep.has(sep)) bySep.set(sep, []); bySep.get(sep)!.push(p); } const seps = [...bySep.keys()].sort((a, b) => a - b); const series = seps.map((sep) => { const q = bySep.get(sep)!.sort((a, b) => a[0] - b[0]); return { name: `sep ${fmt(sep, 2)} s`, x: q.map((p) => p[0]), y: q.map((p) => p[1]), points: true, width: sep === r.sep_delay_s ? 2.6 : 1.4 }; }); return <LineChart series={series} xLabel="ignition delay after separation [s]" yLabel="apogee [ft]" height={240} refs={[{ y: d.target_ft, label: "target", color: "#dc2626" }]} markers={isNum(r.ign_delay_s) ? [{ x: r.ign_delay_s, label: "chosen", color: "#16a34a" }] : []} />; };
  return <div className="mt-2 flex flex-col gap-3">
    ...
    {hq.isLoading ? <div className="text-muted">Loading flight configuration…</div> : hq.error ? <Callout kind="err">{(hq.error as Error).message}</Callout> : hq.data ? <>
      ...
      <div className="grid gap-3 lg:grid-cols-2"><div className="card tight"><div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-muted">Search samples</div>{samplesChart()}</div>
        {hq.data.hist ? <div className="card tight"><div className="mb-1 flex items-center justify-between text-[11px] font-semibold uppercase tracking-wide text-muted"><span>{hq.data.kind === "verified" ? "Verified time history" : "Estimated time history"}</span>{hq.data.kind === "estimated" ? <span className="font-normal normal-case tracking-normal">on-demand flight, not yet verified</span> : null}</div><HistoryChart t={hq.data.hist} mode={mode} onMode={setMode} onHover={(t) => player?.preview(t)} onLeave={() => player?.clearPreview()} /></div> : <div className="card tight"><div className="text-[11px] font-semibold uppercase tracking-wide text-muted">Time history</div><div className="text-[12px] text-muted">no time history yet — run verify, or check sep/ign delays</div></div>}</div>
    </> : null}
  </div>;
}
