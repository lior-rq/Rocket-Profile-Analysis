...
const HELP: Record<string, string> = { "backend": "rasaero_native (RASAero's engine, no VM) | python (RASAero tables) | rasaero (VM GUI) | openrocket (preview)", "rasaero.engine": "where RASAero runs for aero/reference/confirm: auto (native if built) | native | vm", "native.workers": "engine host processes: auto = every core", "native.dt_s": "integration step [s]", "python_sim.workers": "CPU processes for the python backend", "paths.openrocket_jar": "auto = the installed OpenRocket", "paths.jvm": "auto = the JRE next to the jar", "target.apogee_ft": "what the search aims for", "target.tolerance_ft": "solved when |apogee − target| ≤ tolerance", "mass_model.hardware_mass_lb": "dry mass of the whole rocket; empty = .ork masses as-is", "surface_finish": "RASAero surface finish name", "rasaero.engine_name_format": "how RASAero names motors in <Booster1Engine>" };
...
export function SettingsPage() {
  ...
  const val = (f: Field) => (f.key in edits ? edits[f.key] : f.value);
  ...
  const Row = ({ f }: { f: Field }) => { const v = val(f) as any; const changed = f.key in edits; const label = f.key.split(".").slice(1).join(".") || f.key; return <div className={cn("grid grid-cols-[minmax(160px,1fr)_minmax(0,2fr)] items-center gap-3 border-b border-line py-1.5 text-[13px]", changed && "bg-accent-bg/30")}><div><div className="font-mono text-[12px]">{label}</div>{HELP[f.key] ? <div className="text-[11px] text-muted">{HELP[f.key]}</div> : null}</div><div>{f.kind === "boolean" ? <Select className="h-7" value={String(v)} onChange={(e) => set(f, e.target.value === "true")}><option value="true">true</option><option value="false">false</option></Select> : f.kind === "number" ? <Input type="number" step="any" className="h-7" value={v ?? ""} onChange={(e) => set(f, e.target.value === "" ? null : Number(e.target.value))} /> : f.kind === "list" ? <Input className="h-7 font-mono" value={Array.isArray(v) ? v.join(", ") : String(v ?? "")} placeholder="comma separated" onChange={(e) => set(f, e.target.value.split(",").map((x) => x.trim()).filter(Boolean).map((x) => (/^-?\d+(\.\d+)?$/.test(x) ? Number(x) : x)))} /> : <Input className="h-7" value={v ?? ""} placeholder="null" onChange={(e) => set(f, e.target.value === "" ? null : (/^-?\d+(\.\d+)?$/.test(e.target.value) ? Number(e.target.value) : e.target.value))} />}</div></div>; };
  return <div className="flex flex-col gap-4">
    ...
    <Card><Tabs defaultValue={sections[0]}><TabsList>{sections.map((sec) => <TabsTrigger key={sec} value={sec} extra={fields.filter((f) => f.key.split(".")[0] === sec && f.key in edits).length || null}>{sec}</TabsTrigger>)}<TabsTrigger value="__raw">raw YAML</TabsTrigger></TabsList>
      {sections.map((sec) => <TabsContent key={sec} value={sec}>{fields.filter((f) => f.key.split(".")[0] === sec).map((f) => <Row key={f.key} f={f} />)}</TabsContent>)}
