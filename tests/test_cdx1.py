...
def test_launch_site_read(cdx1_file):
    site = cdx1.launch_site(cdx1.load(cdx1_file))
    assert site["altitude_ft"] == 2782 and site["rod_length_ft"] == 22 and site["temperature_f"] == 110


def test_write_batch_and_read_back(tmp_path, cdx1_file):
    t = cdx1.load(cdx1_file)
...
def test_merge_results_detects_unrun_rows(tmp_path, cdx1_file):
    rows = [row(3.0), row(4.5)]
    results = cdx1.read_results(cdx1.write_batch(cdx1.load(cdx1_file), rows, tmp_path / "b.CDX1"))
